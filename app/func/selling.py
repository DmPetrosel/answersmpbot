...
user_cost = 250 * 2 / 10000 * int(config.get("gigachat", "ratio"))


async def start(
    message: types.Message, command: CommandObject, state: FSMContext, bot: MyBot
):
    try:
        await state.clear()
        ...
        await bot.send_message(
            ...
            "Привет, я бот для автоответов на ВБ\nПродолжая пользоваться ботом, вы соглашаетесь на <a href='https://telegra.ph/Politika-konfidencialnosti-AOtvety-06-18'>обработку персональных данных и политикой конфидециальности</a>.",
            parse_mode="html",
        )
        if await get_user(chat_id=message.from_user.id):
            await bot.send_message(
                message.from_user.id,
                "Вы уже зарегистрированы! Для управления ботом воспользуйтесь командами.",
            )
            ...
            if promo_obj == None and arg is not None:
                await bot.send_message(
                    message.from_user.id,
                    "Промокод не найден! Вы сможете ввести его после имени.",
                )
            ...
            if (
                promo_obj
                and promo_obj.expire_date != None
                and usr[message.from_user.id]["expire_date"] < datetime.now().date()
            ):
...
async def get_answer_reg(message: types.Message, state: FSMContext, bot: MyBot):
    ...
    if await state.get_state() == Form.username.state:
        ...
        try:
            if (
                var.lower().strip() == "ямаркетолог"
                or var.lower().strip() == "я маркетолог"
            ):
                usr[message.from_user.id]["marketer"] = True
                usr[message.from_user.id]["promocode"] = "ямаркетолог"
                await state.clear()
            else:
                promo_obj = await get_promo_by_kwargs(
                    promocode=usr[message.from_user.id]["promocode"]
                )
                ...
                if usr[message.from_user.id]["price"] == None:
                    await bot.send_message(
                        message.from_user.id,
                        "Промокод не найден, попробуйте ещё раз: /start",
                    )
                    await state.set_state(Form.promocode)
                else:
                    await state.clear()
        ...
        await bot.send_message(
            message.from_user.id, "Что-то пошло не так, попробуйте ещё раз: /start"
        )
...
async def write_registration(message: types.Message, state: FSMContext, bot: MyBot):
    ...
    logging.info(
        f"REGISTRATION DATA: {usr[chat_id]['username']}, {usr[chat_id]['first_name']}, {usr[chat_id]['promocode']}, {usr[chat_id]['marketer']}"
    )
    if await add_user(
        chat_id=chat_id,
        username=usr[chat_id]["username"],
        first_name=usr[chat_id]["first_name"],
        promocode=usr[chat_id]["promocode"],
        marketer=usr[chat_id]["marketer"],
    ):
        await bot.send_message(chat_id, "Регистрация прошла успешно")
    else:
        await bot.send_message(
            chat_id, str("Регистрация не удалась: \nВозможно, вы уже зарегистрированы")
        )
        return False
...
async def callback_selling(
    callback: types.CallbackQuery, state: FSMContext, bot: MyBot
):
    config.read("config.ini")
    if callback.data == "no_promo_call":
        ...
        await pay_start(
            callback=callback,
            state=state,
            amount=usr[callback.message.chat.id]["price"],
            bot=bot,
        )
        ...
        await bot.send_message(
            ...
            f"Вы получили 300 бонусных рублей, для того, чтобы могли опробовать функции бота.",
        )
        user = await get_user(callback.from_user.id)
        await update_user_by_id(id=user.id, balance=300)
        await bot.send_message(
            callback.from_user.id,
            "Создайте бот в @botfather и вставьте сюда токен бота.",
            reply_markup=how_to_create_bot_kb(),
        )
        ...
        await callback.message.edit_text(
            ...
            reply_markup=del_bot_kb(int(callback.data.split("_")[-1])),
        )
        ...
        await callback.message.edit_text(
            f"Выберете менеджера из списка. Если его в списке нет, вероятно, он не нажал кнопу старт в боте {tbot_data.bot_username}",
            reply_markup=await add_manager_list_kb(tbot_data.bot_username),
        )
        ...
        success = await update_register(
            id=callback.data.split("_")[-1],
            approve=True,
            principal_chat_id=int(callback.message.chat.id),
        )
        if success is not None:
            ...
            await bot.send_message(
                callback.from_user.id, "Что-то пошло не так, попробуйте ещё раз."
            )
        ...
        if managers:
            await callback.message.edit_text(
                "Выберете менеджера для удаления",
                reply_markup=await del_manager_list_kb(tbot.bot_username),
            )
        else:
        ...
        if bot_info is not None:
            await callback.message.edit_text(
                f"Информация о боте {bot_info.bot_username}\n"
        ...
        await bot.send_message(
            callback.from_user.id, "Что-то пошло не так, попробуйте ещё раз: /start"
        )
...
async def my_bots(message: types.Message, state: FSMContext, bot: MyBot):
    ...
    await bot.send_message(
        message.chat.id,
        "Выберете бота для просмотра инфрмации и управления.",
        reply_markup=await my_bots_kb(chat_id=message.from_user.id),
    )
...
async def promo_continue(chat_id, price):
    await bot.send_message(
        ...
        f"ℹ️ Средняя стоимость сообщения 1 Р\n"
        f"ℹ️ 1 000 сообщений будут стоить около 1 000 рублей\n\n",
...
async def get_bot_token(message: types.Message, state: FSMContext, bot: MyBot):
    ...
    if is_exist:
        ...
        return
    result = await bot_init(
        token=message.text.strip(),
        chat_id=int(message.from_user.id),
        managers=[message.from_user.id],
    )
    try:
        list_n, msgs = result
    except:
        list_n = result
    new_bot[message.from_user.id] = (
        {}
    )  # chat_id, token, bot_username, company_name, samples_ans, wb_token
    ...
    if list_n is not None:
        try:
            new_bot[message.from_user.id]["bot_username"] = bot_list[list_n][
                "bot_username"
            ]
            if list_n is not None:
                await bot.send_message(
                    message.from_user.id,
                    f'Токен бота: {message.text.strip()}\n\n Теперь введите API-токен WB.\nОн должен быть сделан с возможностью записи чтобы можно было отвечать на WB отзывы. Должны быть подключены категории "Отзывы" и "Аналитика".',
                )
            await bot_list[list_n]["bot"].send_message(
                message.from_user.id, f"Поздравляем, бот подключён!"
            )
            await add_bot_info(new_bot[message.from_user.id])
            try:
                reg = await get_one_register(
                    chat_id=int(message.from_user.id),
                    bot_username=bot_list[list_n]["bot_username"],
                )
                if reg is not None:
                    await update_register(
                        id=reg.id,
                        chat_id=int(message.from_user.id),
                        approve=True,
                        principal_chat_id=int(message.chat.id),
                    )
                else:
                    await add_register(
                        chat_id=int(message.from_user.id),
        ...
        await bot.send_message(
            message.from_user.id,
            f"Бот не подключён. Попробуйте ещё раз. \n\nВведите токен бота: ",
        )
        await state.set_state(NewBot.get_bot_token)


async def get_wb_token(message: types.Message, state: FSMContext):
    list_n = await get_bot_row(
        bot_username=new_bot[message.from_user.id]["bot_username"]
    )
    ...
    if ping != 200:
        await bot.send_message(
            message.from_user.id,
            'Токен ВБ не авторизован. Поменяйте токен или попробуйте ещё раз. Не забудьте дать разрешения на запись и категории: "Отзывы" и "Аналитика". Введите токен ещё раз.\n',
        )
        await state.set_state(NewBot.get_wb_token)
        return
    elif ping == 200:
        await bot.send_message(
            message.from_user.id, f"API-токен WB: {message.text.strip()}"
        )
        await bot.send_message(message.from_user.id, f"Поздравляем, бот подключён!")
        await bot.send_message(
            message.from_user.id, "Теперь введите название компании (магазина)."
        )
        bot_list[list_n]["wb_token"] = wb_token
        print("\nstate ping 200 ")
        logging.warning(
            f"bot_username = {new_bot[message.from_user.id]['bot_username']}"
        )
        bot_info = await get_one_bot(
            bot_username=new_bot[message.from_user.id]["bot_username"]
        )
        ...
        await bot.send_message(
            message.from_user.id, f"Что-то пошло не так, возможно, проблема с токеном."
        )
    await msg.delete()
...
async def change_wb_token(message: types.Message, state: FSMContext):
    ...
    if bot_info is None:
        await bot.send_message(
            message.chat.id,
            f'Произошла ошибка при получении информации о боте. Воспользуйтесь командами для дальнейших действий или свяжитесь с поддержкой {config.get("support", "support")}',
        )
        await state.clear()
        return
    ...
    if ping != 200:
        ...
        await bot.send_message(
            message.from_user.id,
            "Новый токен установлен. Для дальнейших действий, воспользуйтесь командами.",
        )
        bot_list[list_n]["wb_token"] = wb_token
        await update_bot_info_by_id(id=bot_info.id, wb_token=wb_token)
        await state.clear()
...
async def get_description(message: types.Message, state: FSMContext, bot: MyBot):
    ...
    await bot.send_message(
        message.from_user.id,
        "Описание и компания добавлены! Не забудьте добавить менеджеров в бота /addm . Менеждер, предварительно, должен нажать /start в Вашем боте.",
    )
    await state.clear()
...
async def pay_command(message: types.Message, state: FSMContext, bot: MyBot):
    ...
    await bot.send_message(
        message.from_user.id, "💵 Введите суму в рублях, например: 3 000 или 2500"
    )
...
async def pay_start(
    callback: types.Message, state: FSMContext, amount: int, bot: MyBot
):
    await state.clear()
    ...
    try:
        ...
        logging.error(
            f"pay_start: Ошибка при выполнении команды /pay: {e}\n{traceback.print_exc()}"
        )
        await bot.send_message(
            callback.from_user.id, "Произошла ошибка при обработке команды!"
        )
        current_state = await state.get_state()
        if current_state is not None:
            await state.clear()
...
async def pay_sum(message: types.Message, state: FSMContext, bot: MyBot):
    ...
    if amount < 0:
        await bot.send_message(
            message.chat.id,
            "Сумма не может быть отрицательной. Введите сумму ещё раз (в рублях).",
        )
        await state.set_state(PayState.enter_sum)
        return
    amount_cop = int(amount * 100)
...
async def process_pre_checkout_query(
    pre_checkout_query: types.PreCheckoutQuery, bot: MyBot
):
    try:
...
async def process_successful_payment(
    message: types.Message, state: FSMContext, bot: MyBot
):
    await message.reply(
    ...
    await add_money_stat(
        chat_id=message.from_user.id,
        amount=(message.successful_payment.total_amount / 100),
        invoice_id=message.successful_payment.telegram_payment_charge_id,
        invoice_payload=message.successful_payment.invoice_payload,
    )
    user = await get_user(message.from_user.id)
    data = cast_state.get(message.chat.id, {"bonus": False})
    ratio = 1
    if data.get("bonus") == True:
        ratio = 1.2
    ...
    if user.is_payed_first_time == False:
        ...
        await update_user_by_id(
            id=temp_promo.user.id,
            balance=temp_promo.user.balance
            + ((message.successful_payment.total_amount / 100) * 0.3),
            payout=temp_promo.user.payout
            + ((message.successful_payment.total_amount / 100) * 0.3),
        )
...
def register_selling_handlers(dp: Dispatcher):
    dp.callback_query.register(
        ...
        in (
            ...
            "pay_call",
            "no_pay_call",
            "how_to_create_bot_call",
            "cancel_call",
        )
        or c.data.startswith("mc"),
    )
    dp.message.register(start, Command(commands=("start", "restart")), State(state="*"))
    dp.message.register(
        get_answer_reg, StateFilter(Form.first_name, Form.username, Form.promocode)
    )
    ...
    dp.message.register(
        new_promo,
        StateFilter("promo_name_state", "promo_price_state", "promo_expire_date_state"),
    )

    dp.callback_query.register(
        callback_marketer, lambda c: c.data in ("my_promos", "create_promo")
    )
